...
crawl=$(sed -n 's/\r$//; s/^KOLOBOK CRAWL frames=60 .* fps10=\([0-9][0-9]*\)$/\1/p' "$perf_log")
if [[ -z "$result" || -z "$paced" || -z "$profile" || -z "$crawl" ]]; then
...
if (( crawl < minimum_crawl10 )); then
    printf '386DX-40 credit crawl: FAIL (%d.%d fps; minimum %d.%d)\n' \
        "$((crawl / 10))" "$((crawl % 10))" \
        "$((minimum_crawl10 / 10))" "$((minimum_crawl10 % 10))" >&2
    exit 1
fi
...
printf '386DX-40 credit crawl: PASS (%d.%d fps; 30 Hz music/render target)\n' \
    "$((crawl / 10))" "$((crawl % 10))"
