...
{
    GameState game;GameInput input;u32 a,b;unsigned i;
    if(assets->map_w!=96||assets->map_h!=11||assets->level.required_red!=6)return 0;
...
static int capture_frame(AssetPack*assets,const char*kind)
{
    ...
    else if(kind&&!stricmp(kind,"home"))video_render_ending(&game,60);
...
int main(int argc,char**argv)
{
    ...
    int running=1,music_requested=1,test_mode=0,benchmark_mode=0,capture_mode=0,invalid_code=0;
    ...
    for(i=1;i<argc;++i){if(!stricmp(argv[i],"-nosound"))sound_on=0;else if(!stricmp(argv[i],"-nomusic"))music_requested=0;else if(!stricmp(argv[i],"-selftest"))test_mode=1;else if(!stricmp(argv[i],"-benchmark"))benchmark_mode=1;else if(!stricmp(argv[i],"-capture")){capture_mode=1;if(i+1<argc&&argv[i+1][0]!='-')capture_kind=argv[++i];}}
    if(capture_mode&&capture_kind&&(!stricmp(capture_kind,"forest")))stage=1;
    else if((capture_mode&&capture_kind&&(!stricmp(capture_kind,"deep")||!stricmp(capture_kind,"home")||!stricmp(capture_kind,"frozen")))||benchmark_mode)stage=2;
    if(!assets_load_bank(&assets,"KOLOBOK.DAT",bank_names[stage],level_names[stage],error,sizeof(error))){fprintf(stderr,"KOLOBOK: %s\n",error);return 2;}
    ...
    while(running){wait_for_frame(&next,&remainder);speaker_tick();music_tick();++ui_ticks;
        ...
        if(ui==UI_INTRO){if(key_pressed(KEY_ESCAPE)){ui=UI_PLAY;keyboard_clear_edges();}else if(key_pressed(KEY_ENTER)||ui_ticks>=135){++intro_scene;ui_ticks=0;if(intro_scene>=4)ui=UI_PLAY;keyboard_clear_edges();}if(ui==UI_INTRO){video_render_intro(&assets,intro_scene,ui_ticks);video_present();continue;}}
        ...
        if(ui==UI_ENDING){if(key_pressed(KEY_ENTER)){ui=UI_CREDITS;ui_ticks=0;}video_render_ending(&game,ui_ticks);video_present();continue;}
        if(ui==UI_CREDITS){if(key_pressed(KEY_ENTER)||key_pressed(KEY_ESCAPE)){video_shutdown();assets_free(&assets);stage=0;if(!load_stage(&assets,stage,error,sizeof(error)))break;if(!video_init(&assets))break;game_init(&game,&assets);ui=UI_TITLE;music_play(MUSIC_TITLE);}else{video_render_ending(&game,ui_ticks);video_present();}continue;}
        if(game.game_over){if(key_pressed(KEY_ENTER)){video_shutdown();assets_free(&assets);stage=0;if(!load_stage(&assets,stage,error,sizeof(error)))break;if(!video_init(&assets))break;game_init(&game,&assets);ui=UI_TITLE;music_play(MUSIC_TITLE);}video_render_game_over(&game);video_present();continue;}
